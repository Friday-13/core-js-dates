...
function getNextFriday(date) {
  ...
  let toNextFriday = (12 - currentDay) % 7;
  if (toNextFriday === 0) toNextFriday += 7;
  return new Date(date.getTime() + toNextFriday * 24 * 60 * 60 * 1000);
}
...
function getWeekNumberByDate(date) {
  ...
  week += (endDate.getTime() - startDate.getTime()) / 7 / (24 * 60 * 60 * 1000);
  return week;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const currentDate = new Date(period.start.split('-').reverse());
  const endDate = new Date(period.end.split('-').reverse());
  const schedule = [];
  let workSequence = 0;
  while (currentDate <= endDate) {
    while (workSequence < countWorkDays && currentDate <= endDate) {
